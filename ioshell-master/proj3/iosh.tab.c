...
{
      14,     0,    -1,    15,     5,    -1,     7,    16,    -1,     8,
      16,    -1,     9,    16,    -1,    10,    -1,     3,    16,    -1,
       4,    16,    -1,    -1
};
...
/* Line 1806 of yacc.c  */
#line 44 "iosh.y"
    {run((yyvsp[(1) - (2)].command_val)); printf("%s", prompt);}
    break;
...
void run(command* curCmd){
	switch(curCmd -> commandType){
		case 1:
			free(prompt);
			prompt = malloc(sizeof(*(curCmd->args)));
			strcpy(prompt, *(curCmd->args));
			break;
	}
}
...
command* addArg(char* arg, command* curCmd){
	char* curArg = curCmd->args;
	/*loop to an empty arguemny*/
	while(curArg != NULL){
		curArg++;
	}
	curArg = malloc(sizeof(arg));
	strcpy(curArg, arg);
...
command* setPromptCmd(command* curCmd){
	curCmd->commandType = 1;
	return curCmd;
}

command* setDebugCmd(command* curCmd){
	curCmd->commandType = 2;
	return curCmd;
}

command* chdirCmd(command* curCmd){
	curCmd->commandType = 3;
	return curCmd;
}
